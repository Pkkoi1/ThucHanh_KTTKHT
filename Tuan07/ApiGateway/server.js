...
axiosRetry(axiosInstance, {
  ...
  retryDelay: (retryCount, error) => {
    ...
    const serviceName =
      Object.entries(services).find(([name, serviceUrl]) =>
...
app.get("/health", async (req, res) => {
  try {
    ...
    const serviceChecks = Object.entries(services).map(
      async ([service, url]) => {
        try {
...
async function checkServiceWithBreaker(url) {
  try {
    ...
    console.log(
      `${serviceName.padEnd(12)}: \x1b[32mRunning and Healthy\x1b[0m`
    );
    return "OK";
  } catch (error) {
    ...
    if (error.code === "ECONNREFUSED") {
      console.log(
        `${serviceName.padEnd(
          12
        )}: \x1b[31mNot Started (tried ${totalRetries} attempts)\x1b[0m`
      );
      return "Not Started";
    }
    console.log(
      `${serviceName.padEnd(
        12
      )}: \x1b[31mDown (tried ${totalRetries} attempts)\x1b[0m`
    );
    return "Down";
  }
}
